	...
	if (options.async === true) {
		...
		if (typeof spec === "string" || spec instanceof Buffer) {
			...
			try {
				...
				/** error, go on and try yaml parser **/
			}
			...
			try {
				if (options.includeBodies && methodStruct.requestBody) {
					resultMethod.body = getConvertedSchema(
						"request",
						methodStruct.requestBody,
						schemaOptions
					);
				}

				if (options.includeResponses && methodStruct.responses) {
					resultMethod.responses = buildResponses(
						methodStruct.responses,
						schemaOptions
					);
				}
			} catch (e) {
...
function buildResponses(responses, schemaOptions) {
	...
	for (statusCode in responses) {
		...
		resultResponses[statusCode] = getConvertedSchema(
			"response",
			responseData,
			schemaOptions
		);
