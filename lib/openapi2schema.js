var mergeAllOf = require('json-schema-merge-allof'),
    ...
    jsYaml = require('js-yaml'), fs = require('fs');
...
function buildPaths(paths, options, schemaOptions) {
  var pathName, pathStruct, methodName, methodStruct, resultMethod, result = {};
    ...
    for (methodName in pathStruct) {
      ...
      try {
        if (options.includeBodies && methodStruct.requestBody) {
          resultMethod.body = getConvertedSchema(
              'request', methodStruct.requestBody, schemaOptions);
        }
...
function buildResponses(responses, schemaOptions) {
  var statusCode, responseData, resultResponses = {};
...
function getConvertedSchema(type, struct, options) {
  ...
  schema = mergeAllOf(JSON.parse(JSON.stringify(schema)), {
    resolvers : {
      example : function(values, path, mergeSchemas,
                         options) { return values[0]; }
    }
  });
...
function isEmptyObj(obj) { return Object.keys(obj).length > 0 ? false : true; }
